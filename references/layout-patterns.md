...
12. [内容密度限制](#内容密度限制)
13. [文字防碰撞规则](#文字防碰撞规则) — 形状选择 / 旋转余量 / z-index 层级（memphis 实战）
14. [快速检查清单](#快速检查清单)
...
> 以下规则来自 template-07-memphis 一连串文字碰撞的根因复盘。装饰形/旋转/层叠是这类高装饰风格的常见坑，三类问题都不会触发 lint，只能靠生成时的硬约束预防。

### 规则 1：承载多行文字的容器只用对文字友好的形状

**要点**：承载多行文字的容器（气泡 / 卡片 / 面板）用**圆角矩形 / 椭圆**。`padding > border-radius` 时文字必定落在内接区内，不会越出形外。装饰形（ameoba / 星形 / 水滴 / 不规则 blob / 任意 svg 形状）拉伸后内接区太窄，**只做点缀，不承载多行文字**。

**案例**：memphis voices 证言气泡原用 ameoba 形状，多行证言文字越出形外。换成圆角矩形 + `padding:44px 46px 40px` + `border-radius:36px` 解决——padding（44/46/40）大于 border-radius（36），文字稳定在内接区。

| 容器形状 | 内接区 | 能否承载多行文字 |
|---|---|---|
| 圆角矩形 / 椭圆（padding > border-radius） | ≈ 全宽减 padding | ✅ |
| 正圆 / 椭圆（内容贴边） | 中部窄、两端尖 | ⚠️ 仅单行短文字 |
| ameoba / 星形 / 水滴 / 不规则 blob | 拉伸后内接区很窄 | ❌ 只做点缀 |

### 规则 2：旋转块下方预留 `sin(θ)×块宽` 的下沉余量

**要点**：`transform:rotate(θ)` 的实色色块（标题高亮 / 贴纸 / 徽章 / 装饰标签），其下方若有文字，垂直间距必须预留 `sin(θ)×块宽` 的余量。旋转后块的角会向下/向上"探出"包络盒，否则下沉的角压到下方文字。

**案例**：memphis 封面 `THINGS.` 标题块 `rotate(1deg)`、宽约 465px，下沉约 `sin(1°)×465 ≈ 8px`，压到下方副标题。根因是 section reset（见下文 SKILL.md 间距规范）把副标题的 `margin-top` 吞成 0；副标题 `margin-top` 用 inline `style="margin-top:20px !important"` 恢复后解决。

**估算**：θ 越大余量越大。θ=1° ≈ 块宽×0.0175；θ=5° ≈ 块宽×0.087。块越宽、旋转越大，下沉越明显。

### 规则 3：z-index 层级表，强调标签不能盖住内容可读区

**要点**：建立明确的 z-index 层级，强调标签（VS / 徽章 / 角标）若叠在内容文字上方，**必须确认不盖住信息文字的可读区**——要么错开到无文字带，要么确认它在文字行的间隙。

| 层级 | z-index | 用途 |
|---|---|---|
| 装饰 | 1 | 背景纹理、几何点缀、形状 |
| 内容 | 3 | 标题、正文、卡片、proof object |
| 强调标签 | 5+ | VS / 徽章 / 角标 / stamp |

**案例**：memphis voices 的 `VS` 标签（z5）原浮在两段证言文字（z2）上方压字。把 VS 移到两气泡之间的无文字带解决——同一元素、同一 z-index，错开位置即可，不必降 z-index。
