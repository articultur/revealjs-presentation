...
- **数据曲线禁止 `T` 平滑二次贝塞尔**：`T` 会反射上一段控制点，容易让趋势线末端上翘或产生非数据形态。趋势/损失/指标曲线优先用 `polyline` 或显式 `C` 三次贝塞尔，并确保最后一段单调、末端方向符合数据含义。
- **SVG 图表文字不继承 stroke**：图表根 SVG 可以有 `stroke` 画坐标轴，但 `<text>` 必须 `stroke:none; paint-order:fill;`，否则小字号标签会糊。
