...
| 层 | 脚本 | 性质 | 失败处置 |
|---|---|---|---|
| **地板（合规）** | `grade-gate.js` 全绿（十一门禁 G1-G11 合一） | 硬约束、机器判、不可人工放行 | 任一红灯 → 回 P5 重生成 |
...
## 十一门禁（G1-G11，grade-gate.js 合一）

`grade-gate.js <file>` 一次跑完十一门禁，退出码 1 = 任一红灯，**机器判 verdict，禁止人工放行**（案例：v15.0 把「15 页」压成 12 页时主观放行导致翻车）。
...
```bash
# 地板（合规）—— 交付前必跑，全绿才放行
node scripts/grade-gate.js <file>                      # 十一门禁合一 = 合规地板
...
# 图片驱动 deck 专项 —— 城市/旅游/美食/产品摄影必跑
node scripts/audit-image-assets.js <file>                # 断图、满版图放大/低清、超宽低高图、重复 hero、背景漂移
node scripts/audit-image-assets.js <file> --json         # JSON 输出（blocker 使 passed=false）
...
**视觉能力自检（必做）**：Read 一张截图后，若返回内容是文件路径/URL/空描述而非像素视觉内容（某些 harness/proxy 的会话模型读不到图像素，Read 只回路径），即视为无视觉能力 → 此 fallback 判定无效，必须记 `visual-verdict passed=null（dry-run·会话模型无视觉能力）`，**不得声称「Claude 读图判定无 blocker」**。

**反幻觉锚点**：别靠「Read 返回了一段描述」就认定有视觉能力——某些 harness/proxy 会返回模型基于文件名/alt/上下文编造的「伪视觉判读」文字（幻觉），看起来像像素读取实则不是。必须用**文件名/alt 推断不出的具体细节**做客观验证：要求说出如「右上角 stamp 旋转几度」「封面第 2 行第 4 个字」「第 3 页图表主色调」——答不出/答错/泛泛而论 = 无视觉能力，记 `passed=null`。

**实证记录**：iteration-2 实测 4/6 agent 在无视觉 harness 里仍报告「视觉模型判定通过」，全是无效判定；iteration-3 eval6 with_skill 仍误报 `passed=true`，5/6 诚实记 null——证明反幻觉锚点必要。

**阻断含义汇总**（任一触发都不能交付）：

| 脚本 | 阻断条件 |
|------|----------|
| `grade-gate.js` | 退出码 1 = 十一门禁任一红灯（机器判，不可人工放行） |
...
| `audit-image-assets.js` | 退出码 1 = 图像资产 blocker：断图、满版图被放大、满版图低于 1280×720、超宽低高图硬塞 hero、重复 hero/chapter 大图 |
...
| `unclear-proof-object` / `unreadable-label` / `chart-does-not-explain-claim` / `photo-does-not-explain-claim` / `repeated-photo` / `low-quality-photo` / `theme-fragmentation` / `weak-design-impact` / `weak-visual-hierarchy` | visual-verdict.js | LLM 视觉语义评审（专业/发布会级阻断，快速模式需报告） |
...
eval runner 用 `grade-gate.js --json` 的 `passed` 字段作为客观断言——`passed: true` 仅当十一门禁全部通过。禁止 grader 用主观判断覆盖机器 verdict（v15.0 把「15 页」压成 12 页时曾因主观放行翻车）。
