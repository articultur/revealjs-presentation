...
| 层 | 脚本 | 性质 | 失败处置 |
|---|---|---|---|
| **地板（合规）** | `grade-gate.js` 全绿（八门禁 G1-G8 合一） | 硬约束、机器判、不可人工放行 | 任一红灯 → 回 P5 重生成 |
...
## 八门禁（G1-G8，grade-gate.js 合一）

`grade-gate.js <file>` 一次跑完八门禁，退出码 1 = 任一红灯，**机器判 verdict，禁止人工放行**（案例：v15.0 把「15 页」压成 12 页时主观放行导致翻车）。
...
```bash
# 地板（合规）—— 交付前必跑，全绿才放行
node scripts/grade-gate.js <file>                      # 八门禁合一 = 合规地板
...
| 模式 | 地板 | 天花板 | 视觉评审 |
|------|------|--------|----------|
| **快速模式** | `grade-gate.js` 全绿（必跑） | `design-strength-check.js` 四维达标（必跑） | ≥12 页或含密集数据/图表时加跑 `visual-qa.js --annotate-overflow` |
| **专业模式** | P4 生成后立即跑 | P5/P6 跑 | P6 必跑 `visual-qa.js --annotate-overflow --show-fragments` |
| **发布会级** | 必跑 | 必跑 + `--golden` 对比 | 必跑逐页截图审阅 + PPTX 导出证明 + `test-launch-grade-contract.js` |
...
eval runner 用 `grade-gate.js --json` 的 `passed` 字段作为客观断言——`passed: true` 仅当八门禁全部通过。禁止 grader 用主观判断覆盖机器 verdict（v15.0 把「15 页」压成 12 页时曾因主观放行翻车）。
