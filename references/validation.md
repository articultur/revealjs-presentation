...
node scripts/test-spatial-integrity.js <file>           # 表面坐标系 / SVG text 裁切门禁（G10）
...
| `test-spatial-integrity.js` | 退出码 1 = proof object 漂出承载面、physicalContract 违约、页标/北向标/pin 压关键对象、SVG 文字被 viewBox 裁切 |
...
**视觉语义盲区**：固定脚本只能稳定抓几何事实；当问题是“图示不知道在表达什么”“文字合法但读不清”“图表不解释 action title”时，跑 `visual-verdict.js`。它使用同一套截图与固定 rubric，把视觉模型输出保存为 `visual-verdict.json`。`--dry-run` 只证明截图和 prompt 已生成，不代表模型判定通过。
...
| `surface/proof-object drift` / `physicalContract drift` / `page-label frame collision` / `north-mark heading collision` / `pin-object collision` / `marker-label collision` / `svg text clipping` | test-spatial-integrity.js | 机器门禁（G10） |
