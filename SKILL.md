...
5. **自检**：`node scripts/grade-gate.js <file>` 全绿；若页数 ≥12 或含密集数据/图表布局，加跑 `visual-qa.js --annotate-overflow` 逐页审阅
...
选模板看主题的**形状/动作**，不是行业关键词——同行业的两个主题可能要不同模板（"AI 历史"是历程→01，"AI 系统"是系统→02）。先走下表，命中就**复用该种子 + 改叙事骨架/proof object**（失败门禁 #9：不能只换字体颜色）。
...
```bash
node scripts/grade-gate.js <file>                      # 八门禁合一（lint + validate + label-overlap + lint-main-claim + evidence-ledger + color-role + contrast-aa + canvas-fill），机器判 verdict
...
**P4 后必跑**：`grade-gate.js <file>`（八门禁合一，见 §P4），**机器判 verdict，禁止人工"minor"放行**。八门禁各覆盖不同缺陷域，**不可互相替代**。任一红灯回 P5 重生成。
