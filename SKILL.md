...
本 skill 的总集成契约见 `references/layered-architecture.md`。外部项目只作为取舍后的方法来源，不作为默认运行依赖：吸收 `Powerpoint-fancy-design` 的内容优先 / safe zone / role 分类，吸收 `ppt-agent-skill` 的风格和图表词库，吸收 academic PPT 思路的 ghost deck / action title，吸收 impeccable 的 anti-slop 约束，最终仍保持本仓库的单文件 Reveal.js + 六门禁管线。
...
```mermaid
graph TB
    subgraph 四套质量体系["四套质量体系"]
        direction LR
        S1["关键约束 8 项<br/>§1-§8"]
        S2["设计硬规则 10 条"]
        S3["失败门禁 13 条<br/>#1-#13"]
        S4["P0-P6 7 阶段"]
    end

    subgraph grade-gate["grade-gate.js 六门禁（机器判 verdict）"]
        direction LR
        G1["G1 lint P0=0<br/>抓设计规则"]
        G2["G2 validate total=0<br/>抓渲染溢出"]
        G3["G3 label-overlap exit 0<br/>抓标签泄露/重叠"]
        G4["G4 lint-main-claim exit 0<br/>抓 pin 代主命题"]
        G5["G5 evidence-ledger exit 0<br/>抓无标签指标"]
        G6["G6 color-role exit 0<br/>抓颜色层级倒挂"]
    end

    subgraph 专项["专项脚本（补充失败门禁）"]
        T1["test-pin-collision → #13"]
        T2["test-reference-contract → #11"]
        T3["visual-qa → #7"]
    end
...
快速模式只需「轻量 ghost deck + Theme-to-Design Router 六行 + 关键约束 + 硬规则 P0」；专业模式叠加 Phase；发布会级再叠加 `references/launch-grade.md`。

### 三种工作模式

| 模式 | 何时用 | 行为 |
|------|--------|------|
| **快速模式（默认）** | 用户只说"做个 PPT"或没具体要求 | 收集 4 要素 → 轻量 ghost deck + 设计语法 → 直接生成 HTML → 六门禁 → 交付说明；不强制 7 题访谈 |
...
| P2 | 输出方案 | ◐ | 内容结构、视觉方向 |
| P3 | 设计评审 | ● | 反模式检查 + 优化方向 |
| P4 | 生成初稿 | ● | HTML + **六门禁**（`grade-gate.js` 全绿 = lint P0=0 **且** validate.js total=0 **且** test-label-overlap.js exit 0 **且** test-lint-main-claim.js exit 0 **且** test-evidence-ledger.js exit 0 **且** test-color-role.js exit 0，机器判 verdict，不可手动放行） |
...
- **交付前必须过六门禁**：`node scripts/grade-gate.js <file>` 全绿（lint P0=0 + validate total=0 + label-overlap exit 0 + lint-main-claim exit 0 + evidence-ledger exit 0 + color-role exit 0），缺一不可。机器判 verdict，禁止人工放行（案例见 §2 VP_TOP）。P4 生成后立刻跑，任一红灯 = 回 §2 拆页/降文字。G1-G6 覆盖不同缺陷域——lint 抓设计规则违规，validate 抓真实渲染溢出，label-overlap 抓标签泄露/重叠，lint-main-claim 抓 pin 代主命题，evidence-ledger 抓无标签指标，color-role 抓颜色层级倒挂——**不可互相替代**。
- **切勿破坏 reveal 的 section 堆叠/隐藏**：生成 HTML 时**不要**给 section 强制 `position: relative`——会覆盖 reveal.css 的 `position:absolute`，导致 section 进文档流垂直堆叠 → overflow:hidden 截断 → 除首页外全空白（v15.2 实测 slide 3 top=1397，已回滚）。
...
```bash
node scripts/grade-gate.js <file>                      # 六门禁合一（lint + validate + label-overlap + lint-main-claim + evidence-ledger + color-role），机器判 verdict
...
| 脚本 | 阻断条件 |
|------|----------|
| `grade-gate.js` | 退出码 1 = 六门禁任一红灯（机器判，不可人工放行） |
...
**P4 后必跑（六门禁，任一不满足回 P5 重生成）**：`node scripts/grade-gate.js <file>`（内含 lint P0=0 + validate total=0 + label-overlap exit 0 + lint-main-claim exit 0 + evidence-ledger exit 0 + color-role exit 0），**机器判 verdict，禁止人工"minor"放行**。六者覆盖不同缺陷域——lint 抓设计规则，validate 抓真实渲染溢出，test-label-overlap 抓标签泄露/重叠，lint-main-claim 抓 pin 代主命题，evidence-ledger 抓无标签指标，color-role 抓颜色层级倒挂，**不可互相替代**。
...
**评估/基准测试框架集成**：eval runner 在 grading 阶段必须用 `grade-gate.js --json` 作为客观断言——`passed: true` 仅当六个 gate 全部 `passed: true`。禁止 grader 用主观判断覆盖机器 verdict（案例见 §2 VP_TOP）。
