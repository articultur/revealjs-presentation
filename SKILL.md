...
六门禁（G1 lint → G2 validate → G3 label-overlap → G4 lint-main-claim → G5 evidence-ledger → G6 color-role）自动覆盖关键约束与设计硬规则；失败门禁由六门禁及 test-pin-collision / test-reference-contract 等专项脚本联合检查。快速模式 = 轻量 ghost deck + 六行设计语法 + 六门禁；专业模式叠加 P0-P6；发布会级再叠加 `references/launch-grade.md`。
...
- **交付前必须过六门禁**：`grade-gate.js <file>` 全绿（G1-G6 全过，见 §P4），缺一不可。机器判 verdict，禁止人工放行（案例见 §2 VP_TOP）。P4 生成后立刻跑，任一红灯 = 回 §2 拆页/降文字。
...
**P4 后必跑**：`grade-gate.js <file>`（六门禁合一，见 §P4），**机器判 verdict，禁止人工"minor"放行**。六门禁各覆盖不同缺陷域，**不可互相替代**。任一红灯回 P5 重生成。
...
**评估框架集成**：eval runner 用 `grade-gate.js --json` 的 `passed` 字段作为客观断言——`passed: true` 仅当六门禁全部通过。禁止 grader 用主观判断覆盖机器 verdict（案例见 §2 VP_TOP）。
