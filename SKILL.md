...
description: |
  Generate visually distinctive reveal.js presentations (single self-contained HTML, no install needed). Use this skill whenever the user mentions PPT, slides, presentation, 幻灯片, 课件, 汇报, 演讲, keynote, pitch deck, make a deck, need slides, or asks to turn a topic/outline/document/数据 into slides — also when they say 做报告, 写课件, 培训材料, 方案介绍, 年终总结, 答辩, 述职, 产品发布, 技术分享, 营销方案, 路演, 汇报材料, 方案汇报, 工作总结, 季度汇报, 结题报告, OKR/复盘汇报, 融资/BP/提案, 架构评审, 战略汇报, 新版本/发版说明, or hint at presenting information visually. Also use when refreshing or polishing an existing reveal.js HTML, exporting to PPTX/PDF, fixing layout overflow or pin-overlap in slides, fixing broken slides, fixing text cutoff in slides, 幻灯片显示不全, 文字被裁切, setting up the presentation tool, or generating decks in a specific visual style (editorial archive, dark-tech cockpit, architectural plate, live keynote, field notebook, or one of the 5 seed templates). Even if the user does not explicitly say "slides", invoke this skill if their ask is to communicate a structured argument visually to an audience.
---
...
八门禁（G1 lint → G2 validate → G3 label-overlap → G4 lint-main-claim → G5 evidence-ledger → G6 color-role → G7 contrast-aa → G8 canvas-fill）自动覆盖关键约束与设计硬规则；失败门禁由八门禁及 test-pin-collision / test-reference-contract 等专项脚本联合检查。快速模式 = 轻量 ghost deck + 六行设计语法 + 八门禁；专业模式叠加 P0-P6；发布会级再叠加 `references/launch-grade.md`。
...
| P5 | 优化迭代 | ● | 按规模执行优化（详见 references/pipeline-phases.md §P5） |
...
- **每页一个 `<section>`**；section 级 flex/grid 必须加 `class="deck-flex"` 或 `class="deck-grid"`（Reveal 会把 present section 的 inline `display` 改成 `block`——**所以写在 stylesheet 里的 section `display:flex` 会被静默覆盖成 dead code，你以为在居中其实没有**；用 `deck-flex`/`deck-grid` 类才能在 reveal override 后重新生效）
...
- **页面过渡只用 `fade`/`slide`，禁 `convex`/`concave`/`zoom`**：3D 过渡给页面套透视，扭曲 `getBoundingClientRect` → `visual-check.js` 报画布尺寸混杂（如 1229×691 夹 1199×752）。所有页画布尺寸**必须一致**，这是不变量（**G8 test-canvas-fill.js 机器查**：每页 section 必须占满 720 画布，不能是内容高度的矮盒）。要"活泼"用 fragment 动效，别换过渡（详见 `references/visual-check.md`、`references/motion-delight.md`）
- **交付前必须过八门禁**：`grade-gate.js <file>` 全绿（G1-G8 全过，见 §P4），缺一不可。机器判 verdict，禁止人工放行（案例见 §2 VP_TOP）。P4 生成后立刻跑，任一红灯 = 回 §2 拆页/降文字。
- **切勿破坏 reveal 的 section 堆叠/隐藏**：用**弱选择器** `.reveal section{position:relative}` 是无害的——reveal.css 的更强选择器（0,2,1）会覆盖它，等于 dead fallback（06/07/08 种子都这么写，正常）。**真正危险的是用 `!important` 或加强选择器去强行覆盖 reveal 的 `position`**——那才会让 section 进文档流垂直堆叠 → overflow:hidden 截断 → 除首页外全空白（v15.2 实测 slide 3 top=1397，已回滚）。同理：要给 present section 垂直居中，用 `.reveal section.present{display:flex!important}`（只命中 present，不碰 .past/.future），别 blanket-force 所有 section。
- **Pin 定位上下文**：pin 的 `position:absolute` 自动相对最近 positioned 祖先（reveal 的 absolute section），无需额外干预；若 section 退回 static（非 positioned），pin 相对 BODY 全叠视口左下角 = test-label-overlap 报泄露。
...
用户给页数（"15 页左右"、"做 8 页"）时，最终 `<section>` 数与目标的偏差必须 **≤1 页**。iteration-1 实测：v15.0 把"15 页"压成 12 页、"12 页"压成 9 页（两处都欠 3 页）——根因是「先紧后松」被误读成"页数也紧"。**内容预算（§2）管的是单页密度，不是总页数**。页数不够就拆页（多一页留白页比挤一页强），不要靠删内容凑数（拆页优先，见 §2）。
...
- **历史/数据主题：具体日期、参数、价格、基准分数本身就是 proof object，必须用真实值 + 来源**。G5 证据门禁靠加 source label 满足，**绝不靠把数字软化为"约/示意/持平/大致"**——软化数字 = 丢掉设计感最关键的"具体性"。`design-strength-check.js` 的 contentSpecificity 维会盯这个。
...
```bash
node scripts/grade-gate.js <file>                      # 八门禁合一（lint + validate + label-overlap + lint-main-claim + evidence-ledger + color-role + contrast-aa + canvas-fill）= 合规地板
node scripts/grade-gate.js --json <file>               # JSON 输出（供 eval 框架消费）
node scripts/design-strength-check.js <file>           # 设计强度四维度量（尺度/用色/张力/隐喻）= 设计天花板，advisory
node scripts/design-strength-check.js <file> --golden <ref.html>   # 与金标准 deck 对比设计强度（回归守卫）
node scripts/element-quality-check.js <file>            # 元素质量四子分（动画/图标/表格/流程图）= 元素级天花板，advisory
...
node scripts/test-lint-main-claim.js <file>               # 主命题进场门禁（自动检测 pin 承担唯一主题）
node scripts/test-evidence-ledger.js <file>               # 证据台账门禁（精确数字必须有 verified / user-provided / illustrative 标签）
node scripts/test-color-role.js <file>                    # 颜色角色门禁（主命题对比度必须高于 pin/页脚）
node scripts/test-contrast-aa.js <file>                  # 绝对 WCAG AA 对比度（文本≥4.5:1 / 大字≥3:1；对齐 impeccable low-contrast）
node scripts/test-canvas-fill.js <file>                 # section 占满画布门禁（不能是内容高度矮盒；对齐 visual-check 画布一致性）
...
node scripts/test-reference-contract.js                # 种子模板对象契约（gate #11）
...
| 脚本 | 阻断条件 |
|------|----------|
| `grade-gate.js` | 退出码 1 = 八门禁任一红灯（机器判，不可人工放行） |
...
| `test-lint-main-claim.js` | 退出码 1 = pin/页码/脚注承担本页唯一主题（主视觉区缺少等价主张） |
| `test-evidence-ledger.js` | 退出码 1 = 精确数字缺少 verified / user-provided / illustrative 标签 |
| `test-color-role.js` | 退出码 1 = 主命题对比度弱于 pin/页脚文本 |
| `test-contrast-aa.js` | 退出码 1 = 文本低于 WCAG AA（正常文本<4.5:1 / 大字<3:1） |
| `test-canvas-fill.js` | 退出码 1 = section 未占满画布（内容高度矮盒 / 页间画布不一致） |
...
**impeccable audit 覆盖映射**：`/impeccable audit` 查的设计维度，哪些已被我们的门禁自动化、哪些仍是人工/建议：
...
**G6 vs G7 分工**：G6（test-color-role）是**相对**层级检查——主命题对比度必须高于 pin/页脚，用近似亮度（OKLCH L），保证"最重要的事最醒目"；G7（test-contrast-aa）是**绝对** WCAG AA——任何文本对它的实际底色必须 ≥4.5:1（大字 ≥3:1），用真 oklch→sRGB 相对亮度。两者互补、不可互替：G6 过了不代表无障碍达标（这正是 2.96:1 白字粉卡能绿过的原因），G7 补上这个洞。

**P4 后必跑两层**：

- **地板（合规）**：`grade-gate.js <file>` 全绿（八门禁合一，见 §P4）。八门禁是**不可绕过的下限**——无障碍、溢出、证据、画布。任一红灯回 P5 重生成。**但满足地板不是目标**：一个 deck 可以零门禁违规却四维全默认（合规但平庸）。
- **天花板（设计强度）**：`design-strength-check.js <file>` 四维度量（尺度对比/用色投入/构图张力/隐喻贯彻）。它是 advisory（退出码恒 0），但**任一维不达标 = 回炉重做骨架，不是微调**。

**关键认知（防止合规驱动退化）**：门禁是地板，设计强度是天花板，两者**不可互替**。当通过某门禁似乎要削弱设计时（如"满版面板怕对比度不足 → 干脆不做面板"），**正解是找一个既大胆又合规的选择**（深化专色到 AA / 用 `var(--c-fg)` 反相面板），**不是把设计改弱来求合规**。把数字软化为"约/持平"来躲 G5 证据门禁 = 失败；正解是保留真实数字 + 加 source label。参见 `references/design-fundamentals.md` §6。

**专业/发布会级 P6 必跑**；快速模式 ≥12 页或含密集数据时跑：`visual-qa.js --annotate-overflow --show-fragments`，溢出页自动红框标注 → 逐页人工审阅。完整 P4/P6 检查表：`references/pipeline-phases.md`。

**调垂直平衡时跑**：`visual-check.js <file>` 报每页重点重心/跨度/画布一致性（启发式，**非阻断**）。它和 visual-qa 分工不同，冲突时信 visual-qa。指标解读、可接受取舍与假阳性见 `references/visual-check.md`。

**评估框架集成**：eval runner 用 `grade-gate.js --json` 的 `passed` 字段作为客观断言——`passed: true` 仅当八门禁全部通过。禁止 grader 用主观判断覆盖机器 verdict（案例见 §2 VP_TOP）。
